...
function App() {
  ...
  useEffect(
    ...
    [currentTab]
  ); // Add the currentTab state as a dependency to the useEffect hook
  //means jb bhi currentTab state change hoga tb tb ye useeffect function call hoga
  ...
  return (
    <div style={{ margin: 20, textAlign: "center" }}>  
      ...
      <button
        onClick={function () {setCurrentTab(1); }} style={{ color: currentTab === 1 ? "red" : "black" }} > Todo #1 </button>
      <button
        onClick={function () { setCurrentTab(2); }} style={{ color: currentTab === 2 ? "red" : "black" }}>Todo #2</button>
      <button
        onClick={function () { setCurrentTab(3);}} style={{ color: currentTab === 3 ? "red" : "black" }}> Todo #3</button>
      <button
        onClick={function () {setCurrentTab(4);}}style={{ color: currentTab === 4 ? "red" : "black" }}>Todo #4</button>

      {/* Display the title of the current tab */}
      <div>{loading ? "Loading..." : tabData.title}</div>
    </div>
    
  ///Why use functions in onClick?
// React requires the value of the onClick prop to be a function that will be executed when the event (in this case, the button click) occurs. 
// If you were to directly pass 1, 2, 3, or 4 as the value, React wouldn't know how to execute it since those are just values, not callable functions.
  );
}
