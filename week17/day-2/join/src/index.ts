...
const pgClient = new Client(
  "postgresql://neondb_owner:npg_o8lw5jdVmZSp@ep-floral-pond-a84eauue-pooler.eastus2.azure.neon.tech/neondb?sslmode=require"
);
...
// http://localhost:3000/meta?id=1  to run
app.get("/meta", async (req, res) => {
  ...
  const userDetails = await pgClient.query(userDetailsQuery, [id]);

  // Fetch user address
  const userAddressQuery =
    "SELECT city, country, street, pincode FROM addresses WHERE user_id = $1";
  const userAddress = await pgClient.query(userAddressQuery, [id]);
